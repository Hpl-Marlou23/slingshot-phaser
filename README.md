...
The version is configured in `src/config.js` under the `version` key:

| Version | Description | Timer |
| :--- | :--- | :--- |
| **`30s`** | **30 Seconds Version** | ⏱️ Ends automatically after 30 seconds |
| **`full`** | **Full Game** | ❌ Play until win condition |
...
### Build All 4 Gameplay Versions Simultaneously

An automated script is provided to build all versions sequentially without manual config changes:
```bash
node scratch/build-versions.mjs
```
This script will build each version (`v1`, `v2`, `v3`, `v4`) and output them safely to a dedicated directory:
*   `dist-versions/index_v1.html`
*   `dist-versions/index_v2.html`
*   `dist-versions/index_v3.html`
*   `dist-versions/index_v4.html`

All outputs are WebP compressed and optimized to compile well below the **5MB** size limit (approx 2MB per version).
